...
implements IConf {
    ...
    public BigDecimal getPrice(ItemStack itemStack) {
        ConfigurationSection itemNumberMatch;
        ...
        if (result.signum() < 0 && (itemNumberMatch = this.config.getConfigurationSection("worth." + itemStack.getTypeId())) != null && itemNumberMatch.getKeys(false).size() == 1) {
    ...
    public int getAmount(IEssentials ess, User user, ItemStack is, String[] args, boolean isBulkSell) throws Exception {
        ...
        if (requireStack && !stack) {
            throw new Exception(I18n._("itemMustBeStacked", new Object[0]));
        }
        int max = 0;
        ...
        if (amount > max || amount < 1) {
            if (!isBulkSell) {
                user.sendMessage(I18n._("itemNotEnough2", new Object[0]));
                user.sendMessage(I18n._("itemNotEnough3", new Object[0]));
                throw new Exception(I18n._("itemNotEnough1", new Object[0]));
            }
            return amount;
        }
        return amount;
    ...
    public void setPrice(ItemStack itemStack, double price) {
        if (itemStack.getType().getData() == null) {
            this.config.setProperty("worth." + itemStack.getType().toString().toLowerCase(Locale.ENGLISH).replace("_", ""), price);
        } else {
            this.config.setProperty("worth." + itemStack.getType().toString().toLowerCase(Locale.ENGLISH).replace("_", "") + "." + itemStack.getDurability(), price);
        }
