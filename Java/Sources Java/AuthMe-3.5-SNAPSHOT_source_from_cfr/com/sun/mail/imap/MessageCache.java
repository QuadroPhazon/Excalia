...
public class MessageCache {
    ...
    private void ensureCapacity(int newsize, int newSeqNum) {
        if (this.messages != null) ** GOTO lbl4
        this.messages = new IMAPMessage[newsize + 64];
        ** GOTO lbl28
lbl4: // 1 sources:
        if (this.messages.length >= newsize) ** GOTO lbl20
        if (this.logger.isLoggable(Level.FINE)) {
            this.logger.fine("expand capacity to " + newsize);
        }
        newm = new IMAPMessage[newsize + 64];
        System.arraycopy(this.messages, 0, newm, 0, this.messages.length);
        this.messages = newm;
        if (this.seqnums == null) ** GOTO lbl28
        news = new int[newsize + 64];
        System.arraycopy(this.seqnums, 0, news, 0, this.seqnums.length);
        for (i = this.size; i < news.length; ++i) {
            news[i] = newSeqNum++;
        }
        this.seqnums = news;
        if (!this.logger.isLoggable(Level.FINE)) ** GOTO lbl28
...
lbl20: // 1 sources:
        if (newsize < this.size) {
            ...
            for (msgnum = newsize + 1; msgnum <= this.size; ++msgnum) {
                this.messages[msgnum - 1] = null;
                if (this.seqnums == null) continue;
