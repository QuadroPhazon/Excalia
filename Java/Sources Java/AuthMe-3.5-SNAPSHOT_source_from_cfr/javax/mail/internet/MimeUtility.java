    ...
    public static String getEncoding(DataSource ds) {
        ...
        try {
            try {
                ...
                isText = cType.match("text/*");
                i = MimeUtility.checkAscii(is, -1, isText == false);
                switch (i) {
                    case 1: {
                        encoding = "7bit";
                        ** break;
...
lbl49: // 1 sources:
    ...
    public static String getEncoding(DataHandler dh) {
        ...
        if (aos.getAscii() != 1) return "base64";
        return "7bit";
    }
    ...
    private static void doEncode(String string, boolean b64, String jcharset, int avail, String prefix, boolean first, boolean encodingWord, StringBuffer buf) throws UnsupportedEncodingException {
        ...
        if (len > avail && (size = string.length()) > 1) {
            ...
            OutputStream eos = b64 ? new BEncoderStream(os) : new QEncoderStream(os, encodingWord);
            try {
                eos.write(bytes);
                eos.close();
            }
    ...
    static int checkAscii(byte[] b) {
        ...
        for (int i = 0; i < b.length; ++i) {
            if (MimeUtility.nonascii(b[i] & 255)) {
                ++non_ascii;
                continue;
            }
            ++ascii;
